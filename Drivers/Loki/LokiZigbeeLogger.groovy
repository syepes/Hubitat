...
metadata {
  definition (name: "LokiZigbeeLogger", namespace: "syepes", author: "Sebastian YEPES", importUrl: "https://raw.githubusercontent.com/syepes/Hubitat/master/Drivers/Loki/LokiZigbeeLogger.groovy") {
    ...
    if(ip != null && port != null) {
      ...
      try {
        ...
        if (deviceDetails.toInteger()) {
          ...
          if(deviceData?.size() > 0 && deviceData?.containsKey(descData?.name?.trim())) {
            labels['device_id'] = deviceData[descData?.name?.trim()].id
            labels['device_id_net'] = deviceData[descData?.name?.trim()].deviceNetworkId
            labels['device_driver'] = deviceData[descData?.name?.trim()].deviceTypeName
            labels['device_driver_type'] = deviceData[descData?.name?.trim()].type
          }
        }

        String line = "device_name: ${descData.name}, profileId: 0x${Integer.toHexString(descData.profileId)}, clusterId: 0x${Integer.toHexString(descData.clusterId)}, sourceEndpoint: ${descData.sourceEndpoint}, destinationEndpoint: ${descData.destinationEndpoint}, groupId: ${descData.groupId}, lastHopLqi: ${descData.lastHopLqi}, lastHopRssi: ${descData.lastHopRssi}"
...
void connect() {
  ...
  try {
    ...
    interfaces.webSocket.connect("http://localhost:8080/zigbeeLogsocket", pingInterval: 30, headers: [ "Connection": "keep-alive, Upgrade", "Upgrade": "websocket", "Pragma": "no-cache", "Cache-Control": "no-cache"])
    pauseExecution(1000)
  } catch(e) {
...
private void loginHE() {
  ...
  try {
    ...
    Map params = [
      ...
      body: [username: "syepes", password: 'SuckMyB$D']
    ]
  ...
  try {
    ...
    httpGet(params) { resp ->
      ...
      if (resp.getStatus() == 200) {
        ...
        inv?.each { deviceData[it.name] = it }
