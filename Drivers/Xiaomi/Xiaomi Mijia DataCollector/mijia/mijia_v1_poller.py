...
class MijiaV1Poller(object):
    """"
    A class to read data from Xiaomi Mijia Bluetooth sensors.
    """

    def __init__(self, mac, cache_timeout=300, retries=3, adapter='hci0'):
        """
    ...
    def name(self):
        ...
        """
        name = read_ble(self._mac, "0x03", retries=self.retries, timeout=self.ble_timeout, adapter=self._adapter)
        if not name:
            raise BluetoothBackendException("Could not read NAME from Mi Temp sensor %s" % (self._mac))
    ...
    def fill_cache(self):
        ...
        if not firmware_version:
            ...
            self._last_read = datetime.now() - self._cache_timeout + timedelta(seconds=120)
            return
    ...
    def battery_level(self):
        ...
        """
        if (self._battery_level is None) or (datetime.now() - timedelta(hours=1) > self._bat_last_read):
            self._bat_last_read = datetime.now()
    ...
    def parameter_value(self, parameter, read_cached=True):
        ...
        with self.lock:
            if (read_cached is False) or \
                ...
                LOGGER.debug("Using cache (%s < %s)", datetime.now() - self._last_read, self._cache_timeout)

        if self._cache and (len(self._cache) == 14):
            return self._parse_data()[parameter]
        else:
            raise IOError("Could not read data from Mi sensor %s", self._mac)
...
def write_readnotif_ble(mac, handle, value, retries=3, timeout=20, adapter='hci0'):
    ...
    while attempt <= retries:
        ...
        with LOCK:
            LOGGER.debug("Created lock in thread %s", current_thread())
            LOGGER.debug("Running gatttool with a timeout of %s, cmd: %s", timeout, cmd)
        ...
        if res:
            LOGGER.debug("Exit write_readnotif_ble with result (%s)", current_thread())
...
def read_ble(mac, handle, retries=3, timeout=20, adapter='hci0'):
    ...
    while attempt <= retries:
        ...
        res = re.search("( [0-9a-fA-F][0-9a-fA-F])+", result)
        if res:
            LOGGER.debug("Exit read_ble with result (%s)", current_thread())
            return [int(x, 16) for x in res.group(0).split()]
