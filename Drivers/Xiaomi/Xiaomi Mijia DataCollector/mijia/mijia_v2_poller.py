...
class MijiaV2Poller:
    ...
    def __init__(self, mac, backend, cache_timeout=300, retries=3, adapter='hci0'):
        """
    ...
    def firmware_version(self):
        ...
        if (self._firmware_version is None) or (datetime.now() - timedelta(hours=24) > self._fw_last_read):
            ...
            with self._bt_interface.connect(self._mac) as connection:
                ...
                _LOGGER.debug('Received result for handle %s: %s', _HANDLE_READ_FIRMWARE_VERSION, res_firmware)
                res_battery = connection.read_handle(_HANDLE_READ_BATTERY_LEVEL)  # pylint: disable=no-member
                _LOGGER.debug('Received result for handle %s: %d', _HANDLE_READ_BATTERY_LEVEL, res_battery)
